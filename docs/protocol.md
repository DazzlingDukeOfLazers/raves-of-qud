...
| block | shape | notes |
|---|---|---|
| `stats` | object | `name`, `hp`/`hpMax`, `level`, `xp`/`xpFloor`/`xpNext`, `temp`, `qn` (Quickness), `ms` (MoveSpeed), `av`/`dv`/`ma` (Qud's displayed combat values), `weight`/`weightMax`, `water` (fresh-water drams = currency), `hunger`/`thirst` (markup-stripped), `terrain` (zone display name). Every read is guarded — a missing part never fails the snapshot. |
