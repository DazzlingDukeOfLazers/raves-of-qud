...
Namespaces and signatures confirmed by **reflection** against the live build (`6000.0.77f1`). **Not a
stable public API** — re-verify after a Qud update with the reflection probe / `dotnet build` against the
shipped `Assembly-CSharp.dll` (see [`CLAUDE.md`](../CLAUDE.md) for the ilspycmd command). Reflect, don't
grep — string-grepping the DLL lies about casing.
