...
| qud_shape seam | `python3 tools/regression/qud_shape_audit.py` | a BARE `Settings.qud_shape()` guarding a branch with an `else` — that else is unreachable in BOTH modes (user mode renders as a 1:1 clone, so the bare gate is never false), which is how the message log's grouping, Nearby's larger icons and row 4's user-mode heights all became dead code that still read as features |
