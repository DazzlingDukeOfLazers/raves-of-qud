...
#
# RAVES PORT (see PATCHES.md): upstream 4.0.0 left this file at Godot 3;
# ported here. Semantic fix beyond renames: Godot 3's `continue` inside
# `match` fell through to the `_` branch — the path that SKIPPED an
# unhandled chunk's bytes. Godot 4's `continue` targets the while loop,
# which would leave the chunk unread and desync every file containing
# IMAP/LAYR/MATL/MATT/rOBJ chunks; those branches now skip explicitly.
# Palette entries are returned as plain Colors (the old code wrapped them
# in the retired Voxel class).
		...
		match chunk_id:
			...
			"nTRN":
				...
				if info_dict.has("_name"):
					node_name = info_dict["_name"].get_string_from_utf8()
			...
			_:
				# IMAP/LAYR/MATL/MATT/rOBJ and anything else unimplemented:
				# consume the chunk's bytes so the stream stays in sync
...
static func read_file(vox_path : String, merge_voxels := true) -> Dictionary:
	var result := { "error": OK }
	var vox_file := FileAccess.open(vox_path, FileAccess.READ)
...
static func _unpack_transfom(rotation, translation) -> Transform3D:
	...
	if rotation != null:
		# extract bits from packed array
		...
		var correctedBasis = Basis.from_euler(
			Vector3(
				eulerRot.x,
				-eulerRot.z,
				-eulerRot.y
	...
	return Transform3D(Basis(rotX, rotY, rotZ), origin)
...
static func _merge_voxels(tree: Dictionary) -> Dictionary:
	...
	if tree.has("transform"):
		...
		if tree.has("model"):
			...
			# Godot 3's translated() was LOCAL-space; keep that semantic
			transform = transform.translated_local(-center)
