...
| mods    |  5.31 |  3.2% | **1:1 PASS DONE (2026-08-03)**: full Qud reproduction — panel geometry/lines/7×7 dither bands (phase-anchored tiles), pictograph, ┤Mods├ header, row anatomy (thumb frame+ticks, ⠿ glyph, ▪ key/values, `<...>` path elision, chip badges), fresh-open no-selection state, empty preview box, bottom command bar with > cursor. Exporter fixed: version NRE (XRL.Version struct), size round-half-up. Remaining: glyph AA + chip dither phase + title-screen residue (version corner/hint bar → title item) |
